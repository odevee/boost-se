...
from parallel_apply import parallel_apply_embed, parallel_apply_ko
...
class Recommender(torch.nn.Module):
    ...
    def embed_enz(self, h, mask, type):
    ...
    def forward(self, input_embeds_cpd, input_embeds_enz, enz_padding, *args, **kwargs):
        ...
        if self.mf_on:
            ...
            emb_cpd_mf = self.embed(input_embeds_cpd, enz_padding, entity='cpds', type=embtype)#.float()
            emb_enz_mf = self.embed(input_embeds_enz, enz_padding, entity='enzs', type=embtype)#.float()
        ...
        if self.mlp_on:
            ...
            if embtype == 'COMMON' and self.mf_on:  # then we already calculated the embeddings above
                ...
                emb_cpd_mlp = self.embed(input_embeds_cpd, enz_padding, entity='cpds', type=embtype)  # .float()
                emb_enz_mlp = self.embed(input_embeds_enz, enz_padding, entity='enzs', type=embtype)  # .float()
    ...
    # auxiliary tasks
    def predict_ko(self, enz_ids, padding, embtype):
        embed_enz = self.embed(enz_ids, padding, entity='enzs', type=embtype).float()
        return self.ko_predictor(embed_enz)

    def predict_ec(self, enz_ids, ec_i, padding, embtype):
        embed_enz = self.embed(enz_ids, padding, entity='enzs', type=embtype).float()
...
def triplet_loss(triplets, margin, entity='cpds', embtype='MF'):
    ...
    emb_anc, emb_pos, emb_neg = [model.embed(triplets[:, i], None, entity=entity, type=embtype) for i in range(3)]
...
def multi_task_loss(model, output, target, epoch):
    ...
    # auxiliary tasks -> not implemented in this script
    if args.KO and args.EC and args.CC:
        ...
        for typ in model.module.embtypes:                  # e.g. ['MF', 'MLP'] or ['COMMON']
            # enzymes: ko-task
            loss_ko += torch.nn.BCELoss()(model.predict_ko(enzs, padding_masks, embtype=typ), ko_targets)
            # loss_ko += torch.nn.BCELoss()(model.module.predict_ko(enzs, padding_masks, embtype=typ), ko_targets)
            # enzymes: ec-task
            for j, ec_dim in enumerate([(0, lEC1), (lEC1, lEC1 + lEC2), (lEC1 + lEC2, lEC1 + lEC2 + lEC3)]):
                ...
                ec_pred_j = model.predict_ec(enzs, j, padding_masks, embtype=typ)
...
if __name__ == "__main__":
    ...
    # training parameters
    parser.add_argument('--EPOCHS', type=int, default=10)#3500)                # TODO: reconsider and also <-> args.T
    ...
    parser.add_argument('--T', type=float, default=6)#700)                    # TODO: reconsider # loss mixing schedule
    ...
    parser.set_defaults(MF_on=True, MLP_on=True, SEP_EMBEDS=True,
                                    AUTOENC=False, KO=False, EC=False, CC=False)  # TODO: reconsider
