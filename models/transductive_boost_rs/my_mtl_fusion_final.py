"""
BOOST-RS IMPLEMENTATION (mixed data)
"""
...
class Recommender(torch.nn.Module):
    def __init__(self, num_cpd, num_enzyme, hidden_dim, dropout, mf_on, mlp_on, sep_embeds):
        ...
        # self.fp_predictor = MLPModel(input_dim=hidden_dim, hidden_dim=hidden_dim,
        #                                             output_dim=fp_label.shape[1], dropout=dropout,
        #                                             last_layer='sigmoid')
    ...
    #     embed_enz = self.embed(ec_ids, entity='enzs', type=embtype)#.float()
    #     return self.ko_predictor(embed_enz)
    #
    # def predict_ec(self, ec_ids, ec_i, embtype):
    #     embed_enz = self.embed(ec_ids, entity='enzs', type=embtype)#.float()
...
def multi_task_loss(model, output, target, epoch):
    ...
    # # auxiliary tasks
    # aux_loss = 0
    # loss_fp, loss_ko, ec_losses, loss_cc = 0, 0, [0, 0, 0], 0
    ...
    #
    # aux_loss += loss_fp + loss_ec + loss_ko + loss_cc
    #
    # # overall loss
...
def evaluate(model, data, last_train_loss, outfile, epoch=0, index=0, **kwargs):
    with torch.no_grad():
        ...
        for name, ypred in eval_list:
            ...
            ppv = np.nan if name == 'ZEROS' else tp / (
                        tp + fp)  # positive predictive value (= precision); lower bound wrt unknown ground truth
            ...
            if epoch == -1:
                ...
                def save_hits(top_pred_indices):
                    ...
                    out.to_csv(
                        "".join([cwd, '/', args.OUT, '/', outfile, f'_run_{args.REPL}_hits{len(top_pred_indices)}.txt']),
                        index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multi-Task Learning on Real System")
    # system and files
    ...
    parser.add_argument('--RHO', type=float, default=0.5)  # of these, fraction that are observed
    ...
    # model structure
    parser.add_argument('--DIM', type=int, default=512)         # TODO: reconsider
    ...
    with open(args.OUT+'/'+args.OUTF+f"_run_{args.REPL}", 'w') as file:
        ...
        fp_label, ec_label, enzyme_ko_hot = fp_label.to(args.DEVICE), ec_label.to(args.DEVICE), enzyme_ko_hot.to(args.DEVICE).int()
        ...
        for i, (train_index, valtest_index) in enumerate(sss1.split(observed[:, :2], observed[:, 2])):
            for j, (val_index, test_index) in enumerate(
                ...
                val_data = val_data[ ~pd.DataFrame(val_data.numpy()).loc[:, 0].isin(trainset_include_cpds) ].to(args.DEVICE)
                test_data = test_data[ ~pd.DataFrame(test_data.numpy()).loc[:, 0].isin(trainset_include_cpds) ].to(args.DEVICE)
